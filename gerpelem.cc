...
int toDiscScore(double x) {
  if (x < 0) return (int)(x * ITOL - 0.5);
  else return (int)(x * ITOL + 0.5);
}
...
struct less_dp : public binary_function<elem, elem, bool> {
  bool operator()(elem x, elem y) {
    ...
    return (toDiscScore(x.score) < toDiscScore(y.score));
  }
};
...
void markShallow(Vec<pos> &a, double penalize) {
...

{
  ...
  for (int i = 0; i < seqlen; i++) {
    if (!a[i].shallow && !a[i].exclude) fr[Z + toDiscScore(a[i].s)] += punit;
  }
  ...
  for (int i = 1; i <= MAX_LEN; i++) {
    ...
    while (celem < found.size() && found[celem].len == i) {
      found[celem].p = gp[to2d(i%2, toDiscScore(found[celem].score) + ZL, n)];
      celem++;
    }
    for (int si = 0; si < NSHUFFLES; si++) {
      while (scelem[si] < sfound[si].size() &&
	     sfound[si][scelem[si]].len == i) {
	sfound[si][scelem[si]].p = gp[to2d(i%2, toDiscScore(sfound[si][scelem[si]].score) + ZL, n)];
	scelem[si]++;
      }
    } 
  }
...
void readRates(Vec<pos> &a)
{
  ...
  while (true) {
    ...
    if (ptemp.s > maxs) maxs = ptemp.s;
    if (ptemp.s < mins) mins = ptemp.s;
  }
  MAX_SC = toDiscScore(maxs) + 1;
  MIN_SC = toDiscScore(mins) - 1;
...
void readParameters(int argc, char *argv[]) {
  for ( ; argc > 0; argc--, argv++) {
    ...
    case 'h':
      ...
      cout << " -c <chromosome> [default = none]" << endl;
      cout << " -s <start offset> [default = 0]" << endl;
      cout << " -x <suffix>" << endl;
      ...
      cout << " -l <length>" << endl;
      cout << "    \t minimum element length [default = 4]" << endl;
      cout << " -L <length>" << endl;
      ...
      cout << " -e <ratio>" << endl;
      cout << "    \t acceptable false positive rate [default = 0.05]" << endl;
      //      cout << " -u <ratio>" << endl;
			...
			double cstop, ofstream &os)
{
  ...
  for (i = 0; i < f.size(); i++) {
    ...
    if ((cstop < 0 && 2 * sitot >= NSHUFFLES) || (cstop > 0 && sitot > cstop * NSHUFFLES * (i+1)))
      break;

    if (w || cstop > 0) {
      os << chr << '\t' << (f[i].L + rstart - 1) << '\t' << (f[i].L + rstart + f[i].len - 1) << '\t' << f[i].score << '\t' << f[i].p << '\t' << lensum << '\t' << (sitot / (double)NSHUFFLES) << '\t'
	 << (slensum / (double)NSHUFFLES) << endl;
    }
  }
...
int main (int argc, char *argv[]) {
  ...
  //  if (v) cout << mins << "\t" << maxs << "\t" << MIN_SC << "\t" << MAX_SC << endl;

  mNR = computeMedianNR(a);
  if (v) cout << "median neutral rate is " << mNR << endl;

  double penalize = -mNR * penalty;
  if (penalize < mins) {
  ...
  unsigned int stop = findCutoff(ffound, fsfound, -1, efs);
  efs.close();
  ...
  stop = findCutoff(ffound, fsfound, fpr, ofs);
  ofs.close();
